...
- `wowdata init` 创建并验证 `~/.wowdoc` 目录、读取 Skill Reference 随版本发布的默认 source catalog、获取 catalog 声明的官方及第三方产品分支源码，并为每个产品分支当前 head 与最多 10 个 catalog 匹配 Tag 构建 AST 中间体和 SQLite 索引；分支不足 10 个匹配 Tag 时全部构建。命令完成后，本地无需再次联网即可查询这些 snapshot。
...
- “最多 10 个 Tag”按 catalog 中的产品分支分别选择。catalog 使用 Tag 前缀、版本范围、TOC/client 证据和 Git 祖先关系声明归属；按 Git `creatordate` 倒序选择，annotated Tag 使用 tagger 时间，lightweight Tag 使用 Commit 时间，并用 Tag 名作为稳定并列排序键。初始化状态保存实际选中的 Tag 和 resolved Commit，重试期间不因远端新增 Tag 改变本次范围。
- 同一个 Tag/Commit 可以关联多个产品分支或 client；全局源码对象与 AST 只保存一次，同一分支库内的文件级查询事实也只保存一次。某分支没有 10 个匹配 Tag 时不得从其他分支填充；没有匹配 Tag 的分支只构建当前 head。
...
- 每个 Git 仓库维护包含全部远程分支、Tag、可达 Commit 和 blob 的完整 bare mirror。首次同步完整获取这些 Git 对象，后续只 fetch 增量；已存在的 partial mirror 必须在同步时受 fetch 锁保护地补齐并移除 promisor/blob 过滤配置。source sync 完成后，worktree 创建、热/冷 snapshot 构建和查询不得因缺失 Git blob 隐式联网；选中 snapshot 的文件内容仍放入按 SHA-256 去重的对象缓存。
...
- 热 Tag 必须先满足目标产品分支的 Git 可达关系，再满足 catalog 的产品线版本规则。共享祖先上的其他客户端 Tag 不得用于补足 10 个；不足 10 个匹配 Tag 时只建立实际匹配项。
...
- 首次建立 snapshot 时解析全部适用文件。更新时先枚举 Git tree，并通过 repository 内不可变 Git blob OID 到 SHA-256 的已验证别名批量判断内容身份；只读取别名未知的 blob，只对 `parserSchema + indexSchema + language + contentHash` 尚未建立的文件解析并写入查询事实。Git OID 不能替代跨 repository 的 SHA-256 内容身份；相同字节属于不同语言时只共享原始内容对象，不共享语言相关解析结果。
- 相同文件内容跨 source、branch 或 commit 共享 `objects` 与 `ast` 中间体。中间体是可检查的 JSON，包含输入 hash、语言、解析器及 schema 版本、提取节点和来源定位。
- 解析过程一次只需持有单文件 AST，完成提取后释放。默认并发使用进程级全局预算，总解析 worker 根据机器资源限定在 4 到 8 个；不能为每个 snapshot 各自创建 4 到 8 个 worker。初始化并行调度不同产品分支，同一分支按 head、Tag 创建时间由新到旧串行构建，以先发布的新内容供后续旧 Tag 复用。
...
- 每个 catalog 产品分支使用独立的 `indexes/<source-id>/<branch-id>.sqlite`，保存该分支 head、10 个热 Tag、按需建立的冷版本及其规范化内容事实、关系、FTS/定位和 snapshot 关联。路径 ID 来自 catalog 的稳定标识，不能直接信任远端分支字符串拼接路径。
...
- `objects` 与 `ast` 按 SHA-256 跨所有 snapshot 去重。分支库内的 symbols、edges、XML、TOC、asset refs、search docs 与 FTS 行按内容解析单元保存一次，不含 snapshot ID、Commit、Tag、path 或角色；snapshot 只保存 `snapshotId + path + contentHash + role` 文件清单以及确实依赖整仓库的关系，不能为每个版本复制一套相同文件级事实。
...
- 结构化查询与 FTS 查询都必须先固定目标 snapshot，并通过 `snapshot_files` 将候选内容单元限制为该 snapshot 的成员；path 和 role 从映射补回。即使同一 content hash 出现在多个 path，也必须分别返回准确路径；只存在于其他 snapshot 的 FTS 命中不得泄漏到当前结果。
...
- 内容去重属于存储与执行优化。对同一已就绪 snapshot 的相同请求，source/product/ref/Commit 解析、稳定错误、命中代码事实、path、line、excerpt、content hash、关系可信度与确定性排序语义必须保持业务等价。内部 row ID、DB generation 路径、耗时、复用计数和 BM25 原始数值可以变化，但不得降低首条代码参考质量或改变公开 score 字段的含义。
- 默认热范围从 20 个 Tag 缩到 10 个 Tag 是明确的业务变化：原先第 11–20 个热 Tag 不再由 `init` 提前离线构建；用户首次请求时仍按既有冷 snapshot 流程固定相同 Commit、按需构建并长期缓存。
...
- `contents` 与 `snapshot_files`：以 `parserSchema + indexSchema + language + contentHash` 标识内容解析单元，保存 AST hash、语言和文件大小；snapshot 映射只保存 path、content hash 与路径角色。
- `symbols`、`references`、`edges`：Lua/API 定义、引用、调用与继承关系；文件级事实按内容解析单元复用且不固化 path，确实依赖整仓库解析的关系才按 snapshot ID 保存。
...
- `search_docs` 与 FTS5 虚表：符号文档、注释和有界源码 chunk；每个内容解析单元的 FTS 行只建立一次，引用 content hash 与相对行号，不保存 snapshot/path/role，也不成为源码真本。
...
- 每个热 snapshot 必须枚举资产路径并物化其唯一内容对象，使初始化完成后可以离线检查当前 head 与 10 个热 Tag 的素材。冷版本仍按通用冷 snapshot 流程首次获取并长期缓存。
...
- Git 元数据、内容解析单元、AST 和 snapshot 关系必须分层保存：Git mirror 保存 refs、Commit 图及 repository 内 blob OID；单文件 AST 和查询事实仅保存由文件内容与 schema 决定的结构，不写 branch、Tag、Commit、path、role 或 checkout 状态；不可变 snapshot manifest 保存 `sourceId`、`resolvedCommit` 以及每个 path 对应的 Git blob/content hash/AST hash；SQLite catalog 保存 Tag/build 到 snapshot 的关系与发布状态。
- 文件级查询事实按 `parserSchema + indexSchema + language + contentHash` 在目标分支库中去重，snapshot 查询通过 `snapshot_id -> path -> contentHash` 关联这些事实。调用关系等确实依赖整份源码的结果可以按 `snapshot_id` 保存。这样相同语言的相同文件跨 Commit 只解析并建立一次 FTS，不同 Agent 只是在同一只读索引中选择不同 snapshot。
...
- `wowdata init` 在每个 repository 完成一次受 fetch 锁保护的同步后固定本轮全部 resolved Commit，再为不同产品分支创建有界并行任务。每个任务使用独立 detached worktree；同一分支一次只发布一个 snapshot，不同分支 DB 可以并行写。并发任务共享同一个全局解析 worker 预算和取消上下文，不能按任务数放大 CPU 并发。
